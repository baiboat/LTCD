...
################## Load Model and Data ##################
from models.SAM_CD import LSAM_CD as Net
NET_NAME = 'SAM_CD'

from datasets import Levir_CD as Data
DATA_NAME = 'DICE-CE-Loss'
...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--crop_size', required=False, default=(768, 768), help='cropping size')
        ...
        parser.add_argument('--chkpt_path', required=False, default=os.path.join(working_path, 'checkpoints', DATA_NAME, 'SAM_CD_e90_OA99.56_F95.41_IoU94.05.pth') )
        ...
        with torch.no_grad():
              if imgA.shape[0]>opt.crop_size[0] or imgA.shape[1]>opt.crop_size[1]:
                  ...
                  for idx in range(crop_num):
                      ...
                      output, outA_NC, outB_NC = net(tensorA, tensorB)
                      output = (F.sigmoid(torch.chunk(output , 2, dim=1)[0]) + 1 - F.sigmoid(torch.chunk(output, 2, dim=1)[1])) /  2.0
                      #output = torch.chunk(output , 2, dim=1)[0] 
                      ...
                      if opt.TTA:
                          ...
                          output_v = (F.sigmoid(torch.chunk(output_v , 2, dim=1)[0]) + 1 - F.sigmoid(torch.chunk(output_v , 2, dim=1)[1])) /  2.0
                          ...
                          output_h = (F.sigmoid(torch.chunk(output_h , 2, dim=1)[0]) + 1 - F.sigmoid(torch.chunk(output_h , 2, dim=1)[1])) /  2.0
                          ...
                          output_hv = (F.sigmoid(torch.chunk(output_hv , 2, dim=1)[0]) + 1 - F.sigmoid(torch.chunk(output_hv , 2, dim=1)[1])) /  2.0
                          output += output_hv            
                          output = output/4.0
                      pred = output.cpu().detach().numpy().squeeze()>0.4
                      preds.append(pred)
