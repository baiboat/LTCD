...
class Triplet_loss(nn.Module):
    ...
    def __init__(self, reduction='mean', T=1.0):
        ...
        self.loss_f = nn.TripletMarginLoss(margin=1.0, p=2, eps=1e-7)
        
        
    def forward(self, label, x1, x2, using_label=False):
        b,c,h,w = x1.size()
        ...
        x1 = torch.reshape(x1,[b*h*w,c])
        x2 = torch.reshape(x2,[b*h*w,c])
        label = torch.reshape(label,[b*h*w,c])
        loss = self.loss_f(label, x1, x2)
        return loss
